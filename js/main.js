...
function cpuDoOneImmediate(){
  ...
  if(game.phase === PHASES.ROUND0_GUARD){
   ...
   if (pick === null) {
      ...
      logPush(`CPU: P${actorId+1} Round0 狩人守り → フォールバック選択`);
      resolveGuard(actorId, pick2);
      return;
    }
  ...
  if(game.phase === PHASES.GUARD){
    ...
    if(pick === null){
      ...
      logPush(`CPU: P${actorId+1} 狩人守り → フォールバック選択`);
      resolveGuard(actorId, pick2);
      return;
    }
  ...
  if(game.phase === PHASES.BITE){
    ...
    const anyPublicSeerExists =
      game.publicSeerReveal.some(v => typeof v === "number");
    ...
    function biteScore(c){
      if(c.mark === MARK.WHITE && !c.biteFail) return 100; // 白（噛めていない白）
      if(c.mark === MARK.GRAY) return 80;                 // グレー
      if(c.mark === MARK.WHITE && c.biteFail) return 80;  // 噛めなかった白
      if(c.mark === MARK.BLACK) return 50;                // 黒
      return 0;
    }
...
if(bReset){
  ...
  for(const pl of game.players){
    ...
    for(let i=0;i<pl.slots.length;i++){
      ...
      // ★占（全体公開）
      if(isPublicSeerSlot(game, pl.id, i)){
...
function bind(){
  ...
  const lobbySkipBtn = document.getElementById("btnLobbySkip");
  if(lobbySkipBtn){
    lobbySkipBtn.addEventListener("click", ()=>{
      fastForward();
      showLobby(false);
    });
  }
